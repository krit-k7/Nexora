      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          ...
          >
            ...
            <div className="absolute top-0 right-0 w-64 h-64 bg-neutral-600/10 blur-[100px] rounded-full -mr-32 -mt-32" />
            <div className="absolute bottom-0 left-0 w-64 h-64 bg-neutral-600/10 blur-[100px] rounded-full -ml-32 -mb-32" />
            ...
            <div className="relative z-10 overflow-y-auto pr-1 scrollbar-hide">
              <div className="mb-6 text-center">
                <motion.div
                  ...
                  className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-neutral-500/30 bg-neutral-500/10 text-[9px] font-bold text-neutral-400 uppercase tracking-wider mb-3"
                >
                ...
                <p className="text-slate-400 text-[11px] max-w-xs mx-auto font-medium">Choose a supported Stellar wallet to establish your secure identity on the Nexora network.</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {WALLETS.map((wallet, idx) => (
                  <motion.button
                    ...
                    className="flex flex-col items-start p-5 rounded-2xl bg-white/5 border border-white/5 hover:border-neutral-500/50 hover:bg-neutral-500/5 transition-all group text-left relative overflow-hidden"
                  >
                    <div className="absolute top-0 right-0 w-16 h-16 bg-neutral-500/5 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity" />
                    
                    <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center mb-3 text-slate-400 group-hover:text-neutral-400 group-hover:border-neutral-500/30 transition-all">
                      {wallet.icon}
                    </div>
                    
                    <h3 className="text-base font-bold text-white mb-0.5 group-hover:text-neutral-400 transition-colors">{wallet.name}</h3>
                    <p className="text-[10px] text-slate-500 group-hover:text-slate-400 transition-colors leading-tight">{wallet.description}</p>
                    
                    <div className="mt-3 flex items-center gap-2 text-[8px] font-bold uppercase tracking-wider text-neutral-500 opacity-0 group-hover:opacity-100 transition-all translate-x-[-10px] group-hover:translate-x-0">
                      Initialize Link
