...
export const WalletProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    const initKit = async () => {
      try {
        ...
        console.error('[Nexora Context] Kit init failed:', err);
      }
    };
    initKit();
  }, []);
      ...
      if (token && savedWalletType) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            console.log('[Nexora Context] Session restored:', user.address, user.accountType);
            setWallet({
          ...
          console.error('[Nexora Context] Session restore failed:', e);
          setWallet(prev => ({ ...prev, status: 'idle' }));
        }
      }
    };
  ...
  const executeConnect = async (selectedWalletType: string, accountType?: string) => {
    ...
    try {
      ...
      const message = "Authenticate with Nexora Protocol".trim();
      
      // Sign Message
      const signResult = await StellarWalletsKit.signMessage(message);
      console.log("[Nexora Auth] Full Sign Result:", signResult);
      console.log("[Nexora Auth] Result Keys:", Object.keys(signResult || {}));
      ...
      if (typeof signResult === 'string') {
        ...
        if (!signature) {
          console.warn("[Nexora Auth] Signature not found in common fields, using stringified result");
      ...
      if (signature && typeof signature !== 'string') {
        try {
          ...
          console.warn("[Nexora Auth] Signature is not a string and Buffer conversion failed", e);
        }
      }

      if (!signature) throw new Error("Signing failed: No signature returned.");
      
      console.log("[Nexora Auth] Final Signature Payload:", {
        address,
      ...
      console.error('[Nexora Context] Connection error:', err);
  ...
  const connect = (accountType?: string) => {
    console.log('[Nexora Context] connect() triggered with type:', accountType || 'none (trigger identity selector)');
    if (accountType) {
