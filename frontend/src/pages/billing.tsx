        ...
        <main className="relative z-10 container mx-auto px-6 py-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <section className="lg:col-span-2 space-y-8">
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-8 rounded-[2.5rem] glass border-slate-200 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
                <div>
                  ...
                  <div className="text-xs text-slate-600 dark:text-slate-400">Account Type: <span className="text-neutral-600 dark:text-neutral-400 font-bold">{wallet.accountType}</span></div>
                </div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="p-8 rounded-[2.5rem] glass border-slate-200 dark:border-white/5">
                ...
                <div className="mb-8">
                  ...
                  <div className="w-full h-3 bg-slate-200 dark:bg-black/50 rounded-full overflow-hidden">
                    <div className="h-full bg-neutral-500 rounded-full w-[14%]" />
                  </div>
                </div>
            ...
            <aside className="space-y-8">
              <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="p-8 rounded-[2.5rem] bg-neutral-50 dark:bg-neutral-950/20 border border-neutral-100 dark:border-neutral-500/20 glass">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tighter mb-6">Testnet Escrow</h3>
                
                <div className="text-center mb-8">
                  <div className="text-[10px] font-bold text-neutral-600 dark:text-neutral-400 uppercase tracking-wider mb-2">Available Balance</div>
                ...
                <form onSubmit={handleDeposit} className="space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-white dark:bg-black/30 border border-slate-200 dark:border-white/10 rounded-2xl px-6 py-4 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-neutral-500 transition-colors"
                    />
                  </div>
                  <button 
                    type="submit" 
                    ...
                    onClick={playClick}
                    className={`w-full py-4 rounded-2xl text-[11px] font-bold uppercase tracking-wider transition-all ${isDepositing ? 'bg-slate-300 dark:bg-slate-700 text-slate-500 cursor-not-allowed' : 'bg-neutral-600 hover:bg-neutral-500 text-white shadow-lg shadow-neutral-500/20'}`}
                  >
                  ...
                  <p className="text-[9px] text-center text-slate-500 mt-4 leading-relaxed">
                    Depositing funds locks tokens in the Nexora Soroban smart contract. You can withdraw unused credits at any time.
                  </p>
                </form>
              </motion.div>
            </aside>
