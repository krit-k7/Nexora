...
export default function GuestDashboard() {
  ...
  const networkEvents = [
    ...
    { time: '12:41:45', event: 'Cross-chain Sync: Ethereum -> Nexora', type: 'info' }
  ];

  return (
    <AuthGuard allowedAccountTypes={['Guest', 'Developer', 'NodeOperator', 'DAOAdmin']}>
      <div className="min-h-screen bg-white dark:bg-zypher-bg selection:bg-neutral-500/30 text-slate-900 dark:text-white transition-colors duration-300">
        <Head>
          <title>Protocol Observer | Nexora</title>
        </Head>
        ...
        <main className="relative z-10 container py-8">
          <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-8">
            <div className="flex items-center gap-4 mb-6">
              <span className="px-3 py-1 bg-neutral-500/10 text-neutral-600 dark:text-neutral-500 rounded-lg text-[9px] font-bold uppercase tracking-wider border border-neutral-500/20">OBSERVER_UPLINK_ESTABLISHED</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
              <section className="p-6 md:p-10 rounded-[2.5rem] glass border-slate-200 dark:border-white/5 relative overflow-hidden bg-neutral-600/[0.02]">
                <div className="relative z-10">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white uppercase tracking-tighter mb-4 italic">Want to build on Nexora?</h3>
                ...
                <div className="absolute top-0 right-0 w-1/2 h-full opacity-10 pointer-events-none">
                  <div className="w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-neutral-500 to-transparent blur-3xl" />
                </div>
              </section>
                ...
                <div className="space-y-4">
                  {networkEvents.map((event, i) => (
                    <motion.div 
                      ...
                      transition={{ delay: i * 0.1 }}
                      className="flex items-center gap-6 p-6 rounded-3xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 group hover:border-neutral-500/30 transition-all"
                    >
                      <span className="text-[10px] font-mono font-bold text-slate-400 group-hover:text-neutral-500 transition-colors">{event.time}</span>
                      ...
                      <span className={`text-[8px] font-bold uppercase tracking-wider ${event.type === 'success' ? 'text-emerald-500' : 'text-neutral-500'}`}>
                        {event.type}
                      </span>
                    </motion.div>
                  ))}
            ...
            <aside className="space-y-8">
              ...
              <div className="p-10 rounded-[3.5rem] bg-slate-900 dark:bg-white text-white dark:text-black shadow-2xl relative overflow-hidden group">
                 ...
                 <div className="absolute -bottom-10 -right-10 w-32 h-32 bg-neutral-500/20 blur-3xl group-hover:bg-neutral-500/40 transition-colors" />
              </div>
            </aside>
          </div>
        </main>
        ...
        <footer className="container mx-auto px-6 py-20 mt-20 border-t border-slate-200 dark:border-white/5">
           <div className="flex flex-col md:flex-row justify-between items-center gap-8">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.4em]">Nexora Protocol v4.2.0-Alpha</div>
