...
export default function DeveloperDashboard() {
  ...
  const [webhooks, setWebhooks] = useState([
    { id: '1', url: 'https://myapp.com/api/nexora-webhook', events: ['EXECUTION_COMPLETED'] }
  ]);
          ...
          <AnimatePresence>
            {notifications.map(n => (
              <motion.div
                ...
                className={`p-4 rounded-xl border-2 glass backdrop-blur-xl flex items-center justify-between gap-3 ${n.type === 'error' ? 'border-red-500/30 text-red-600 dark:text-red-400' : 'border-neutral-500/30 text-neutral-600 dark:text-neutral-400'}`}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className={`w-2 h-2 rounded-full ${n.type === 'error' ? 'bg-red-500' : 'bg-neutral-500'} animate-pulse`} />
             ...
             <div className="bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 p-8 rounded-[3rem] backdrop-blur-3xl shadow-2xl">
                <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                   <div className="space-y-1">
                      <span className="px-3 py-1 bg-neutral-500/10 text-neutral-600 dark:text-neutral-400 rounded-lg text-[9px] font-bold uppercase tracking-wider border border-neutral-500/20">DEV_ENVIRONMENT_ALPHA</span>
                   ...
                   <div className="flex gap-4 items-center">
                      ...
                      {['overview', 'builder', 'api', 'webhooks'].map((tab) => (
                        <button
                          ...
                          className={`px-8 py-3 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all ${activeTab === tab ? 'bg-neutral-600 text-white shadow-xl shadow-neutral-600/30' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5'}`}
                        >
                          {tab}
                        </button>
                      ))}
          ...
          <AnimatePresence mode="wait">
                    ...
                    <table className="w-full text-left">
                      <tbody>
                        {(isDemoMode ? demoRules : rules).map((rule: any) => (
                          <tr key={rule._id} className="border-b border-slate-200 dark:border-white/5 group hover:bg-neutral-600/[0.01] transition-colors">
                            <td className="p-8">
                            ...
                            <td className="p-8 text-right">
                              {isDemoMode ? (
                                ...
                                <button 
                                  ...
                                  className="px-8 py-3 bg-neutral-600 text-white rounded-xl text-[9px] font-bold uppercase tracking-wider shadow-xl shadow-neutral-600/20 hover:bg-neutral-500 transition-all"
                                >
                                  Trigger Proof
                                </button>
                              )}
            ...
            {activeTab === 'builder' && (
              <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }}>
                <form onSubmit={handleCreateRule} className="p-10 rounded-[3.5rem] bg-neutral-50 dark:bg-neutral-900/10 border border-neutral-100 dark:border-neutral-500/10 shadow-2xl">
                  <div className="mb-10">
                    <h3 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tighter">Logic Architect_</h3>
                    <p className="text-neutral-600 dark:text-neutral-400/60 text-[10px] font-bold uppercase tracking-wider mt-2">Design trustless multi-chain predicates</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
                     <div className="lg:col-span-3 space-y-6">
                        ...
                        <textarea 
                          ...
                          rows={10}
                          className="w-full bg-slate-50 dark:bg-black/60 border border-slate-200 dark:border-white/10 rounded-[2rem] px-8 py-6 text-neutral-600 dark:text-neutral-400 font-mono text-sm outline-none shadow-inner"
                        />
                     </div>
                  ...
                  <div className="mt-12 flex justify-end gap-4 border-t border-slate-200 dark:border-white/5 pt-8">
                     <button type="submit" disabled={submitting} className="px-12 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-950 rounded-2xl font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-neutral-600 hover:text-white transition-all shadow-2xl">
                ...
                <div className="p-8 rounded-[2.5rem] glass border-slate-200 dark:border-white/5">
                  ...
                  <div className="space-y-4">
                    <a href="http://localhost:5001/api-docs" target="_blank" rel="noreferrer" className="block p-4 bg-slate-50 dark:bg-black/20 rounded-2xl border border-slate-200 dark:border-white/5">
                      <div className="text-[11px] font-bold text-neutral-600 dark:text-neutral-400 uppercase tracking-wider mb-1">Swagger UI</div>
